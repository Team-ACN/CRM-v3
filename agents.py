...
def process_inventory_status(inventory_status):
    """Process the inventoryStatus map into separate columns"""
    if not isinstance(inventory_status, dict):
        return {"available": "", "delisted": "", "hold": "", "sold": ""}
    
    return {
        "available": inventory_status.get("available", ""),
        "delisted": inventory_status.get("delisted", ""),
        "hold": inventory_status.get("hold", ""),
        "sold": inventory_status.get("sold", "")
    }
...
def process_document_batch(docs_batch: List, batch_number: int) -> List[Dict]:
    ...
    for doc in docs_batch:
        try:
            ...
            timestamp_fields = [
                "added", "lastModified", "lastConnected", "lastEnquiry", 
                "lastEnquiryDid", "lastEnquiryReceived", "lastSeen", "lastTried",
                "nextRenewal", "planExpiry", "trialStartedAt", "verificationDate"
            ]
            ...
            # Process inventoryStatus map into separate fields
            inventory_status = process_inventory_status(item.get("inventoryStatus", {}))
            item.update({
                "inventory_available": inventory_status["available"],
                "inventory_delisted": inventory_status["delisted"],
                "inventory_hold": inventory_status["hold"],
                "inventory_sold": inventory_status["sold"]
            })

            # Flatten array and complex fields
            array_fields = [
                "areaOfOperation", "businessCategory", "contactHistory", 
            ...
            for k, v in item.items():
                if k not in ["inventoryStatus"]:  # Skip the original inventoryStatus as we've processed it
...
def fetch_firestore_data_with_pagination(collection_name: str, page_size: int = BATCH_SIZE) -> List[Dict]:
    """Fetch Firestore data with pagination and batch processing"""
    try:
...
def write_to_google_sheet_batch(data: List[Dict], spreadsheet_id: str, sheet_name: str):
    ...
    try:
        ...
        fixed_columns = [
            ...
            # Status Fields
            "activity", "agentStatus", "userType", "admin", "blackListed", "appInstalled",
            # Location & Areas
            "areaOfOperation_flat", "preferedMicromarket", "property",
            # Credits & Usage
            "monthlyCredits", "boosterCredits", "inboundEnqCredits", "inboundReqCredits",
            # Counts
            "noOfEnquiries", "noOfinventories", "noOfleagalLeads", "noOfrequirements",
            # Inventory Status
            "inventory_available", "inventory_delisted", "inventory_hold", "inventory_sold",
            # Trial & Plan Info
            "trialUsed", "trialStartedAt_formatted", "planExpiry_formatted", "nextRenewal_formatted",
            # Engagement
            "communityJoined", "onBroadcast", "onboardingComplete", "notificationPreference",
            # Contact & Connection
            "connectStatus", "contactStatus", "contactHistory_flat", "lastConnected_formatted",
            "lastSeen_formatted", "lastTried_formatted",
            # Enquiry Info
            "lastEnquiry_formatted", "lastEnquiryDid_formatted", "lastEnquiryReceived_formatted",
            # KAM Info
            "kamId", "kamName",
            # Payment & Source
            "payStatus", "paymentHistory_flat", "source",
            # Timestamps
            "added_formatted", "lastModified_formatted",
            # Notes & Extra
            "notes_flat", "extraDetails",
            # Verification Info
            "verifiedBy"
        ]
        ...
        numeric_fields = {
            "firmSize", "boosterCredits", "inboundEnqCredits", "inboundReqCredits",
            "monthlyCredits", "noOfEnquiries", "noOfinventories", "noOfleagalLeads", 
            "noOfrequirements", "added", "lastModified", "lastConnected", "lastEnquiry",
            "lastEnquiryDid", "lastEnquiryReceived", "lastSeen", "lastTried", 
            "nextRenewal", "planExpiry", "trialStartedAt", "verificationDate"
        }
        
        boolean_fields = {
            "admin", "appInstalled", "blackListed", "communityJoined", "onBroadcast",
            "onboardingComplete", "trialUsed", "verified", "inventory_available",
            "inventory_delisted", "inventory_hold", "inventory_sold"
        }
...
def main():
    ...
    try:
        logger.info("🚀 Starting enhanced Firestore to Google Sheets sync...")
        
        # Configuration
        collection_name = "acnAgents"
        spreadsheet_id = "1pkGrC3RQRxVwkEcb8AZyhT3KICKadw0IW9udkQsQh5k"
        sheet_name = "Agents from firebase Sheet"
